...
/**
 * Responsible for acknowledging batch of Events based on the timer's timeout.
 **/
@Stateless
public class DelayedAckEventBatchTimer {
    ...
    public void startAlarmAckTimer(final LinkedList<List<Long>> poIdSubBatches, final long startTime) {
        ...
        delayedAckEventTimer = timerService.createIntervalTimer(0, timeInterval, timerConfig);
        LOGGER.debug("The delayed ack timer for started with {} event batches ", masterPoIdsList.size());
    }
    ...
    public void handleTimeout(final Timer timer) {
        try {
            ...
            // we are considering the time upto 90% of the current cycle time which is delayedAckInterval *60000 *(100-10)/100 which is equal 54000,
            ...
            LOGGER.error("Exception occurred while trying to perform the DelayedACK on Events. DelayedACK will be performed in the next timeout.",
                    exception);
        }
    }
    ...
    private void batchAcknowledge(final List<Long> poIdsBtach) {
        ...
        LOGGER.debug("Delayed ack performed  for {} Events. ", poIdsBtach.size());
    }

    private void cancelDelayedAckEventTimer() {
        if (delayedAckEventTimer != null) {
            try {
                delayedAckEventTimer.cancel();
                delayedAckEventTimer = null;
            } catch (final Exception exception) {
                LOGGER.error("Exception occurred while cancelling the delayedAckEvantTimer", exception);
            }
        }
